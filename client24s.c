...
int validateInput(const char *command, const char *filename,
		...
		if (strcmp(command, "ufile") == 0) {
			...
			while ((bytesRead = fread(buffer, sizeof(char), BUF_SIZE, fp)) > 0) {
				if (i == 0) {
					...
					send(clientSock, line, strlen(line),
					     0);  // Send the line with the initial part of the file content
				} else {
...
int validateInput(const char *command, const char *filename,
                  const char *destinationPath) {
// Validate command
	if (!(strcmp(command, "ufile") == 0 || strcmp(command, "dfile") == 0 ||
	        strcmp(command, "rmfile") == 0 || strcmp(command, "dtar") == 0 ||
...
// Validate filename extension
	if (filename != NULL && !(strstr(filename, ".c") || strstr(filename, ".pdf") ||
	                          strstr(filename, ".txt"))) {
		printf(
		    "Error: Invalid file type. Only .c, .pdf, and .txt files are allowed.\n");
		return -1;
	}
