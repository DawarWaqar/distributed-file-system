...
void processClient(int connectedSock) {
	...
	while (1) {
		...
		if (strcmp(command, "ufile") == 0) {
			...
			snprintf(filePathTarget, BUF_SIZE, "/home/%s/stext/%s/%s", USERNAME,
			...
			snprintf(fullPath, sizeof(fullPath), "/home/%s/stext%s", USERNAME,
			         secondArg + 7);
			...
			if (createTarFileOfFileType(".txt", fullPath, "txtFiles.tar") == 0) {
				...
				snprintf(fullPathWithTar, sizeof(fullPathWithTar), "%s/txtFiles.tar",
				         fullPath);
				...
				if (tarContent) {
					...
					removeFileFromStext("~/stext/txtFiles.tar");
				}
			}
		} else if (strcmp(command, "display") == 0) {
			char smainPath[BUF_SIZE];
			snprintf(smainPath, sizeof(smainPath), "/home/%s/stext%s", USERNAME,
			         secondArg + 7);
			char *txtFiles = retrieveFiles(smainPath, ".txt", BUF_SIZE * 2);
			sendBytes(connectedSock, txtFiles);
		}
